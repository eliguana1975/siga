...
class MobileChecklistController extends Controller
{
    ...
    {
        if ($forbidden = $this->authorizePermission($request, 'controles-unidad.ver')) {
            return $forbidden;
        }

        return response()->json([
            'partes' => ControlUnidad::PARTES,
            'control_unidad_items' => ControlUnidad::CONTROL_UNIDAD,
            'estados_parte' => ControlUnidad::ESTADOS_PARTE,
            'estados_control' => ControlUnidad::ESTADOS_CONTROL,
            'flotas' => Flota::query()
                ->select('id', 'nro_interno', 'dominio', 'estado')
                ->orderBy('nro_interno')
                ->limit(500)
                ->get(),
            'servicios_asignados' => ServicioAsignado::query()
                ->select('id', 'nombre')
                ->orderBy('nombre')
                ->get(),
        ]);
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        $query = ControlUnidad::query()
            ->with([
                'flota:id,nro_interno,dominio,estado',
                'conductorUser:id,name,email',
                'servicioAsignado:id,nombre',
            ])
        ...
        if ($search !== '') {
            $query->where(function ($builder) use ($search) {
                $builder->where('interno', 'like', "%{$search}%")
        ...
        $controles = $query->paginate($perPage)->withQueryString();

        return response()->json($controles);
    }

    public function show(Request $request, ControlUnidad $controlUnidad): JsonResponse
    {
        if ($forbidden = $this->authorizePermission($request, 'controles-unidad.ver')) {
            return $forbidden;
        }
        ...
        return response()->json([
            ...
            'partes_meta' => ControlUnidad::PARTES,
            'control_unidad_meta' => ControlUnidad::CONTROL_UNIDAD,
        ]);
    }

    public function store(Request $request): JsonResponse
    {
        if ($forbidden = $this->authorizePermission($request, 'controles-unidad.crear')) {
            return $forbidden;
        }
        ...
        $servicioAsignado = ServicioAsignado::query()->findOrFail((int) $request->input('servicio_asignado_id'));
        $flota = Flota::query()->findOrFail((int) $request->input('flota_id'));

        $control = DB::transaction(function () use ($request, $servicioAsignado, $flota, $user) {
            $control = ControlUnidad::query()->create([
                'user_id' => $user->id,
                'flota_id' => $flota->id,
                'conductor_user_id' => $user->id,
                ...
                'conductor' => $user->name,
                'kilometraje_actual' => (int) $request->input('kilometraje_actual'),
                'servicio_asignado' => $servicioAsignado->nombre,
                'observaciones_generales' => (string) $request->input('observaciones_generales'),
                'partes' => $this->normalizePartes($request),
                'control_unidad' => $this->normalizeControlUnidad($request),
            ]);

            if ($this->hasNoCumple($control->partes)) {
                $orden = $this->createOrdenTrabajoFromControl($control, $user);
        ...
        return response()->json([
            'message' => 'Check List Vehicular creado correctamente.',
    ...
    private function validateKilometraje($validator, Request $request): void
    {
        $validator->after(function ($validator) use ($request) {
            ...
            $ultimoChecklist = ControlUnidad::query()
                ->where('flota_id', (int) $request->input('flota_id'))
                ->whereNotNull('kilometraje_actual')
                ->max('kilometraje_actual');
            $ultimaOrden = OrdenTrabajo::query()
                ->where('flota_id', (int) $request->input('flota_id'))
    ...
    private function createOrdenTrabajoFromControl(ControlUnidad $control, User $user): OrdenTrabajo
    {
    ...
    private function resolveBaseForOrden(?Empleado $empleado): ?Base
    {
        ...
        if ($empleado->base_id) {
            $base = Base::query()->where('estado', 'activa')->find($empleado->base_id);
        ...
        return Base::query()->where('estado', 'activa')->orderBy('nombre')->first();
    }
