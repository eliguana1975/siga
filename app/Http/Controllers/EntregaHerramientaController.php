...
use App\Models\EntregaHerramientaDetalle;
...
class EntregaHerramientaController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $entregas = EntregaHerramienta::query()
            ->with([
                'empleado:id,nombres,apellidos,numero_doc,tipo_doc',
                'deposito:id,nombre',
                'usuario:id,name',
                'detalles:id,entrega_herramienta_id,articulo_id,cantidad_entregada,condicion_entrega',
        ...
        return view('admin.entregas-herramientas.index', compact('entregas', 'search'));
    }

    public function create(Request $request)
    {
        return view('admin.entregas-herramientas.create', $this->formData($request));
    }
    ...
    public function edit(Request $request, string $id)
    {
        ...
        return view('admin.entregas-herramientas.edit', $this->formData($request, $entrega) + compact('entrega'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $entrega = DB::transaction(function () use ($validated) {
            ...
            foreach ($cantidadesPorArticulo as $articuloId => $cantidadRequerida) {
                ...
                if (! $articulo || ! $inventario || (int) $inventario->cantidad < (int) $cantidadRequerida) {
                    ...
                    throw ValidationException::withMessages([
                        "detalles.{$detalleIndex}.cantidad" => 'La cantidad supera el stock disponible de ' . ($articulo?->nombre ?? 'la herramienta seleccionada') . '.',
                    ]);
                }
            }

            $entrega = EntregaHerramienta::create([
            ...
            foreach ($validated['detalles'] as $index => $detalle) {
                ...
                if (! $articulo) {
                    throw ValidationException::withMessages([
                        "detalles.{$index}.articulo_id" => 'El articulo seleccionado no esta marcado como herramienta.',
                    ]);
                }
                ...
                if (! $inventario instanceof Inventario) {
                    throw ValidationException::withMessages([
                        "detalles.{$index}.cantidad" => 'No existe inventario para la herramienta seleccionada en el deposito indicado.',
                    ]);
                }
        ...
        return redirect()
            ->route('admin.entregas-herramientas.show', $entrega->id)
            ->with('success', 'Entrega de herramientas registrada correctamente.');
    }
    ...
    public function update(Request $request, string $id)
    {
        ...
        DB::transaction(function () use ($id, $validated) {
            $entrega = EntregaHerramienta::query()
                ->with('detalles')
                ->lockForUpdate()
                ->findOrFail($id);
        ...
        return redirect()
            ->route('admin.entregas-herramientas.show', $id)
            ->with('success', 'Entrega de herramientas actualizada correctamente.');
    }
    ...
    public function show(string $id)
    {
        ...
        $documentos = DocumentoOperativo::query()
            ...
            ->where('documentable_type', EntregaHerramienta::class)
            ->where('documentable_id', $entrega->id)
            ->latest()
            ->get();

        return view('admin.entregas-herramientas.show', compact('entrega', 'documentos'));
    }

    public function planilla(string $id)
    {
        return redirect()->route('admin.entregas-herramientas.show', $id);
    }

    public function devolver(Request $request, string $id, string $detalleId)
    {
        $entrega = EntregaHerramienta::findOrFail($id);
        $detalle = EntregaHerramientaDetalle::query()
            ->where('entrega_herramienta_id', $entrega->id)
    ...
    private function formData(Request $request, ?EntregaHerramienta $entrega = null): array
    {
        ...
        return [
            ...
            'herramientas' => $this->herramientasDisponibles($depositoId, $cantidadesActuales),
            'selectedDepositoId' => $depositoId,
        ];
    }

    private function herramientasDisponibles(?int $depositoId, ?Collection $cantidadesActuales = null)
    {
        if (! $depositoId) {
            return collect();
        }
