...
@section('content')
    <div class="page-heading">
        <div class="d-flex justify-content-between align-items-center">
            <div>
                <h3>Ciudades</h3>
                <p class="text-subtitle text-muted">Administrar las ciudades disponibles en el sistema.</p>
            </div>
            <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#createCiudadModal">
        ...
        <section class="section">
            <div class="card">
                <div class="card-header">
                    <h4 class="card-title mb-0">Listado de ciudades registradas</h4>
                </div>
                <div class="card-body">
                    @if (session('error'))
                    ...
                    <form method="GET" action="{{ route('admin.ciudades.index') }}" class="mb-3">
                        <div class="row g-2 align-items-end">
                            <div class="col-12 col-md-8">
                                <label for="search" class="form-label mb-1">Buscar ciudad</label>
                            ...
                            <div class="col-12 col-md-4 d-flex gap-2">
                                ...
                                <a href="{{ route('admin.ciudades.index') }}" class="btn btn-light-secondary w-100">Limpiar</a>
                            </div>
                        </div>
                    </form>
                    ...
                    <div class="row mb-3">
                        <div class="col-12 d-flex flex-column flex-md-row gap-2">
                            <button type="button" class="btn btn-outline-secondary w-100 w-md-auto"
                                onclick="exportTableToExcel('datatable', 'ciudades_registradas')">
                                <i class="bi bi-file-earmark-spreadsheet"></i> Exportar Excel
                            </button>
                            <button type="button" class="btn btn-outline-secondary w-100 w-md-auto"
                                onclick="downloadCSVFromTable('datatable', 'ciudades_registradas')">
                                <i class="bi bi-file-earmark-arrow-down"></i> Exportar CSV
                            </button>
                            <button type="button" class="btn btn-outline-secondary w-100 w-md-auto"
                                onclick="createPDF('datatable', 'ciudades_registradas')">
                    ...
                    @if (!empty($search ?? request('search')))
                        <div class="alert alert-success py-2 mb-3" role="alert">
                            Se encontraron {{ $ciudades->total() }} resultado(s) para "{{ $search ?? request('search') }}".
                        </div>
                    @endif
                        ...
                        <table id="datatable" class="table table-striped mb-0">
                            ...
                            <tbody>
                                @forelse ($ciudades as $ciudad)
                                    <tr>
                                        <td>{{ $ciudades->firstItem() + $loop->index }}</td>
                                        <td>{{ $ciudad->nombre }}</td>
                                        <td class="text-end">
                                            <button type="button" class="btn btn-sm btn-success" data-bs-toggle="modal"
                                                data-bs-target="#editCiudadModal-{{ $ciudad->id }}">
                                                <i class="bi bi-pencil-square"></i>
                                            </button>

                                            <button type="button" class="btn btn-sm btn-danger" data-bs-toggle="modal"
                                                data-bs-target="#deleteCiudadModal-{{ $ciudad->id }}">
                    ...
                    @if ($ciudades->count() > 0)
                        <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-2 mt-3">
                            <small class="text-muted">
                                Mostrando {{ $ciudades->firstItem() }} a {{ $ciudades->lastItem() }} de
                                {{ $ciudades->total() }} registros
                            </small>
                            <div>
                                {{ $ciudades->links('vendor.pagination.bootstrap-5-no-summary') }}
                            </div>
                        </div>
                    @endif
                </div>
    ...
    <div class="modal fade" id="createCiudadModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog">
            <form class="modal-content" method="POST" action="{{ route('admin.ciudades.store') }}">
                @csrf
                <div class="modal-header bg-primary text-white">
                    <h5 class="modal-title" style="color: white">Crear Ciudad</h5>
                    ...
                    <div class="mb-3">
                        ...
                        <div class="input-group">
                            ...
                            <input type="text" name="nombre" id="nombre" class="form-control"
                                value="{{ old('nombre') }}" placeholder="Nombre de ciudad" required>
                        </div>
                    </div>
                </div>
    ...
    @foreach ($ciudades as $ciudad)
        <div class="modal fade" id="editCiudadModal-{{ $ciudad->id }}" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog">
                <form class="modal-content" method="POST" action="{{ route('admin.ciudades.update', $ciudad->id) }}">
                    @csrf
                    @method('PUT')
                    <div class="modal-header bg-success text-white">
                        <h5 class="modal-title" style="color: white">Editar Ciudad</h5>
                        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"
                            aria-label="Close"></button>
                    </div>
                    ...
                    <div class="modal-body">
                        <div class="mb-3">
                            <label for="nombre-{{ $ciudad->id }}" class="form-label">Nombre (*)</label>
                            <div class="input-group">
                                <span class="input-group-text"><i class="bi bi-tags-fill"></i></span>
                                <input type="text" name="nombre" id="nombre-{{ $ciudad->id }}" class="form-control"
                                    value="{{ session('open_modal') === 'editCiudadModal-' . $ciudad->id ? old('nombre', $ciudad->nombre) : $ciudad->nombre }}"
                                    placeholder="Nombre de ciudad" required>
                            </div>
                        </div>
                    </div>
        ...
        <div class="modal fade" id="deleteCiudadModal-{{ $ciudad->id }}" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered">
                <form class="modal-content" method="POST" action="{{ route('admin.ciudades.destroy', $ciudad->id) }}">
                    @csrf
                    @method('DELETE')
                    <div class="modal-header bg-danger text-white">
                        <h5 class="modal-title" style="color: white">Eliminar Ciudad</h5>
                        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"
                            aria-label="Close"></button>
                    </div>
                    ...
                    <div class="modal-body">
                        <p class="mb-0">
                            ¿Está seguro de eliminar la ciudad <strong>{{ $ciudad->nombre }}</strong>?
                        </p>
                    </div>
