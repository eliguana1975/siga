...
@section('content')
    <div class="page-heading">
        <div class="d-flex justify-content-between align-items-center">
            <div>
                <h3>Nueva entrega de ropa y EPP</h3>
                <p class="text-subtitle text-muted">Registra la ropa y los EPP entregados a un empleado.</p>
            </div>
    ...
    <div class="page-content">
        <section class="section">
            <div class="card">
                <div class="card-body">
                    <form method="POST" action="{{ route('admin.entregas-ropa-epp.store') }}">
                        @csrf
                        <div class="row g-3">
                            ...
                            <div class="col-12 col-md-4">
                                ...
                                <select id="deposito_id" name="deposito_id" class="form-select" required data-ppes-url="{{ route('admin.entregas-ropa-epp.create') }}">
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            function optionHtml() {
                return ['<option value="">Seleccione prenda/EPP</option>'].concat(options.map(option => {
                    return `<option value="${escapeHtml(option.id)}">${escapeHtml(option.label)} - Stock ${escapeHtml(option.stock)} ${escapeHtml(option.unidad)}</option>`;
                })).join('');
            }
            ...
            function appendRow(data = {}) {
                ...
                const rowCantidadInput = row.querySelector('input[name$="[cantidad]"]');
                const condicionInput = row.querySelector('input[name$="[condicion_entrega]"]');
                ...
                if (rowCantidadInput && data.cantidad) {
                    rowCantidadInput.value = data.cantidad;
                }

                if (condicionInput && data.condicion_entrega) {
                    condicionInput.value = data.condicion_entrega;
                }
