...
@php
    $canCreateCategorias = auth()->user()?->can('categorias.crear');
    $canEditCategorias = auth()->user()?->can('categorias.editar');
    $canDeleteCategorias = auth()->user()?->can('categorias.eliminar');
    $showCategoriaActions = $canEditCategorias || $canDeleteCategorias;
@endphp
...
@section('content')
    <div class="page-heading">
        <div class="d-flex justify-content-between align-items-center">
            <div>
                <h3>Gestión de Categorías de Productos</h3>
                <p class="text-subtitle text-muted">
                    Administra las categorías utilizadas para organizar los productos registrados en el sistema.
                </p>
            </div>
            @if ($canCreateCategorias)
                ...
                <div class="card-body">
                    ...
                    <div class="row mb-3">
                        <div class="col-12 d-flex flex-column flex-md-row gap-2">
                            <button type="button" class="btn btn-outline-secondary w-100 w-md-auto"
                                onclick="exportTableToExcel('datatable', 'categorias_registradas')">
                                <i class="bi bi-file-earmark-spreadsheet"></i> Exportar Excel
                            </button>
                            <button type="button" class="btn btn-outline-secondary w-100 w-md-auto"
                                onclick="downloadCSVFromTable('datatable', 'categorias_registradas')">
                                <i class="bi bi-file-earmark-arrow-down"></i> Exportar CSV
                            </button>
                            <button type="button" class="btn btn-outline-secondary w-100 w-md-auto"
                                onclick="createPDF('datatable', 'categorias_registradas')">
                    ...
                    @if (!empty($search ?? request('search')))
                        <div class="alert alert-success py-2 mb-3" role="alert">
                            Se encontraron {{ $categorias->total() }} resultado(s) para "{{ $search ?? request('search') }}".
                        </div>
                    @endif
                    ...
                    <div class="table-responsive">
                        <table id="datatable" class="table table-striped mb-0">
                            ...
                            <tbody>
                                ...
                                @empty
                                    <tr>
                                        <td colspan="{{ $showCategoriaActions ? 3 : 2 }}" class="text-center text-muted py-4">No hay categorías registradas.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
    ...
    @foreach ($categorias as $categoria)
        ...
        <div class="modal fade" id="deleteCategoriaModal-{{ $categoria->id }}" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered">
                <form class="modal-content" method="POST" action="{{ route('admin.categorias.destroy', $categoria->id) }}">
                    @csrf
                    @method('DELETE')
                    ...
                    <div class="modal-body">
                        <p class="mb-0">¿Está seguro de eliminar la categoría <strong>{{ $categoria->nombre }}</strong>?</p>
                    </div>
