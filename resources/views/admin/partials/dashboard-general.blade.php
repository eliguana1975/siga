<section class="row">
    <div class="col-12 col-lg-4">
        <div class="row">
            <div class="col-12 col-sm-6 col-lg-12"><div class="card"><div class="card-body px-4 py-4"><h6 class="text-muted font-semibold">Total de vehiculos</h6><h4 class="font-extrabold mb-0">{{ $fleetStats['total'] }}</h4></div></div></div>
            <div class="col-12 col-sm-6 col-lg-12"><div class="card"><div class="card-body px-4 py-4"><h6 class="text-muted font-semibold">Vehiculos activos</h6><h4 class="font-extrabold mb-0">{{ $fleetStats['activo'] }}</h4></div></div></div>
            <div class="col-12 col-sm-6 col-lg-12"><div class="card"><div class="card-body px-4 py-4"><h6 class="text-muted font-semibold">Vehiculos de baja</h6><h4 class="font-extrabold mb-0">{{ $fleetStats['baja'] }}</h4></div></div></div>
            @if ($fleetStats['mantenimiento'] > 0)
                <div class="col-12 col-sm-6 col-lg-12"><div class="card"><div class="card-body px-4 py-4"><h6 class="text-muted font-semibold">En mantenimiento</h6><h4 class="font-extrabold mb-0">{{ $fleetStats['mantenimiento'] }}</h4></div></div></div>
            @endif
        </div>
    </div>
...
<section class="row">
    <div class="col-12 col-lg-4">
        <div class="row">
            <div class="col-12 col-sm-6 col-lg-12"><div class="card"><div class="card-body px-4 py-4"><h6 class="text-muted font-semibold">Pendientes</h6><h4 class="font-extrabold mb-0">{{ $workOrderStats['pendiente'] }}</h4></div></div></div>
            <div class="col-12 col-sm-6 col-lg-12"><div class="card"><div class="card-body px-4 py-4"><h6 class="text-muted font-semibold">En proceso</h6><h4 class="font-extrabold mb-0">{{ $workOrderStats['en_proceso'] }}</h4></div></div></div>
            <div class="col-12 col-sm-6 col-lg-12"><div class="card"><div class="card-body px-4 py-4"><h6 class="text-muted font-semibold">Completadas</h6><h4 class="font-extrabold mb-0">{{ $workOrderStats['completada'] }}</h4></div></div></div>
            <div class="col-12 col-sm-6 col-lg-12"><div class="card"><div class="card-body px-4 py-4"><h6 class="text-muted font-semibold">Canceladas</h6><h4 class="font-extrabold mb-0">{{ $workOrderStats['cancelada'] }}</h4></div></div></div>
        </div>
    </div>
    <div class="col-12 col-lg-8">
        <div class="card">
    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                ...
                <a href="{{ route('admin.servicios-kilometraje.index') }}" class="btn btn-primary"><i class="bi bi-speedometer2"></i> Ver control completo</a>
            </div>
            <div class="card-body">@include('admin.partials.dashboard-servicios')</div>
        </div>
    </div>
</section>
            ...
            <div class="card-body">
                <div class="table-responsive">
                    <table id="vencimientos-verificaciones-table" class="table table-striped align-middle mb-0">
                        ...
                        <tbody>
                            @forelse ($vencimientosVerificaciones as $vencimiento)
                                <tr>
                                    <td class="fw-semibold">{{ $vencimiento['interno'] }}</td>
                                    <td>{{ $vencimiento['dominio'] }}</td>
                                    <td>{{ $vencimiento['vehiculo'] ?: '-' }}</td>
                                    <td>{{ $vencimiento['tipo'] }}</td>
                                    <td>{{ $vencimiento['fecha_vencimiento']?->format('d/m/Y') }}</td>
                                    <td>{{ $vencimiento['dias_restantes'] }} dia(s)</td>
                                    <td>{{ $vencimiento['comprobante'] ?: '-' }}</td>
                                    <td>
                                        <span class="badge {{ $vencimiento['dias_restantes'] <= 3 ? 'bg-light-danger' : 'bg-light-warning' }}">
                                            Proximo
                                        </span>
                                    </td>
                                </tr>
            ...
            <div class="card-body">
                <div class="table-responsive">
                    <table id="vencimientos-carnets-table" class="table table-striped align-middle mb-0">
                        ...
                        <tbody>
                            @forelse ($vencimientosCarnetsConducir as $vencimiento)
                                <tr>
                                    <td class="fw-semibold">{{ $vencimiento['empleado'] }}</td>
                                    <td>{{ $vencimiento['tipo_empleado'] ?: '-' }}</td>
                                    <td>{{ $vencimiento['categoria_carnet'] ?: '-' }}</td>
                                    <td>{{ $vencimiento['fecha_vencimiento']?->format('d/m/Y') }}</td>
                                    <td>{{ $vencimiento['dias_restantes'] }} dia(s)</td>
                                    <td>{{ $vencimiento['telefono'] ?: '-' }}</td>
                                    <td>
                                        <span class="badge {{ $vencimiento['dias_restantes'] <= 3 ? 'bg-light-danger' : 'bg-light-warning' }}">
                                            Proximo
                                        </span>
            ...
            <div class="card-body">
                <div class="table-responsive">
                    <table id="vencimientos-matafuegos-table" class="table table-striped align-middle mb-0">
                        ...
                        <tbody>
                            @forelse ($vencimientosMatafuegos as $vencimiento)
                                <tr>
                                    <td>
                                        @if ($vencimiento['orden_id'])
                                            #{{ $vencimiento['orden_id'] }}
                                        @else
                                            -
                                        @endif
                                    </td>
                                    <td class="fw-semibold">{{ $vencimiento['interno'] }}</td>
                                    <td>{{ $vencimiento['dominio'] }}</td>
                                    <td>{{ $vencimiento['articulo'] }}</td>
                                    <td>{{ $vencimiento['numero'] ?: '-' }}</td>
                                    <td>{{ $vencimiento['fecha_vencimiento']?->format('d/m/Y') }}</td>
                                    <td>{{ $vencimiento['dias_restantes'] }} dia(s)</td>
                                    <td>
                                        <span class="badge {{ $vencimiento['dias_restantes'] <= 7 ? 'bg-light-danger' : 'bg-light-warning' }}">
                                            Proximo
                                        </span>
                                    </td>
                                </tr>
